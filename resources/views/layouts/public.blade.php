    ...
    <nav class="navbar" id="navbar">
            ...
            <div class="flex items-center justify-between h-16">
                ...
                <a href="{{ route('beranda') }}" class="flex items-center gap-3">
                    @if(!empty($profil_layout['logo_bem']))
                        <img src="{{ asset('storage/'.$profil_layout['logo_bem']) }}"
                             alt="Logo BEM" class="h-10 w-10 object-contain rounded-xl">
                    @else
                    ...
                    @endif
                    <div>
                        <div class="text-white font-bold text-sm leading-tight">{{ $profil_layout['nama_bem'] ?? 'BEM Polbis' }}</div>
                        <div class="text-amber-500 text-xs leading-tight">{{ $profil_layout['nama_kampus'] ?? 'Politeknik Bisnis Digital Indonesia' }}</div>
                    </div>
                </a>
                ...
                <div class="hidden md:flex items-center gap-1">
                    ...
                    <div class="relative group">
                        <button class="nav-link flex items-center gap-1 {{ request()->routeIs('struktur') || request()->routeIs('ormawa') ? 'active' : '' }}">
                            Organisasi
                            <svg class="w-3.5 h-3.5 opacity-60 group-hover:rotate-180 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                            </svg>
                        </button>
                        <!-- Dropdown Panel -->
                        <div class="absolute top-full left-0 mt-1 w-48 invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all duration-200 z-50">
                            <div class="bg-slate-900/95 backdrop-blur-md border border-white/10 rounded-xl shadow-2xl overflow-hidden p-1">
        ...
        <div id="mobile-menu" class="hidden md:hidden border-t border-white/10">
            <div class="px-4 py-3 flex flex-col gap-1">
                <a href="{{ route('beranda') }}" class="nav-link block {{ request()->routeIs('beranda') ? 'active' : '' }}">Beranda</a>
                <a href="{{ route('tentang') }}" class="nav-link block {{ request()->routeIs('tentang') ? 'active' : '' }}">Tentang</a>
                <a href="{{ route('kegiatan') }}" class="nav-link block {{ request()->routeIs('kegiatan*') ? 'active' : '' }}">Kegiatan</a>
                <div class="pl-0">
                    <div class="text-slate-500 text-xs px-3 pt-2 pb-1 uppercase tracking-wider">Organisasi</div>
                    <a href="{{ route('struktur') }}" class="nav-link block pl-5 {{ request()->routeIs('struktur') ? 'active' : '' }}">↳ Struktur BEM</a>
                    <a href="{{ route('ormawa') }}" class="nav-link block pl-5 {{ request()->routeIs('ormawa') ? 'active' : '' }}">↳ Ormawa</a>
                </div>
                <a href="{{ route('kontak') }}" class="nav-link block {{ request()->routeIs('kontak') ? 'active' : '' }}">Kontak</a>
            </div>
        </div>
    </nav>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-6 py-8">
                ...
                <div class="flex items-center gap-4">
                    @if(!empty($profil_layout['logo_bem']))
                        <img src="{{ asset('storage/'.$profil_layout['logo_bem']) }}"
                             alt="Logo BEM" class="h-12 w-12 object-contain rounded-xl flex-shrink-0">
                    @else
                        <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-amber-500 to-amber-600 flex items-center justify-center flex-shrink-0 shadow-lg">
                            <span class="text-gray-900 font-black text-sm">BEM</span>
                        </div>
                    @endif
                    <div>
                        ...
                        <div class="text-amber-500/70 text-xs mt-1">Periode {{ $profil_layout['periode'] ?? '2025/2026' }}</div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-4">
                    <div class="text-right">
                        ...
                        @if(!empty($profil_layout['alamat']))
                            <div class="text-slate-500 text-xs mt-1 truncate max-w-xs">{{ Str::limit($profil_layout['alamat'], 50) }}</div>
                        @endif
                    </div>
                    @if(!empty($profil_layout['logo_kampus']))
                        <img src="{{ asset('storage/'.$profil_layout['logo_kampus']) }}"
                             alt="Logo Kampus" class="h-12 w-12 object-contain rounded-xl flex-shrink-0">
                    @else
                        <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-indigo-500 to-indigo-700 flex items-center justify-center flex-shrink-0 shadow-lg">
