...
<div class="page-header">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <p class="text-slate-400 mt-3 max-w-xl">Berbagai kegiatan dan program yang telah, sedang, dan akan dilaksanakan oleh BEM Polbis.</p>
    </div>
</div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-wrap gap-3 mb-10 reveal">
                <a href="{{ route('kegiatan') }}"
                   class="px-4 py-2 rounded-full text-sm font-semibold transition-all
                          {{ !request('kategori') ? 'bg-amber-500 text-gray-900' : 'bg-white/05 text-slate-400 hover:text-amber-500 border border-white/10' }}">
                    Semua
                </a>
                @foreach($kategori_list as $kat)
                    <a href="{{ route('kegiatan', ['kategori' => $kat]) }}"
                       class="px-4 py-2 rounded-full text-sm font-semibold transition-all capitalize
                              {{ request('kategori') === $kat ? 'bg-amber-500 text-gray-900' : 'bg-white/05 text-slate-400 hover:text-amber-500 border border-white/10' }}">
                        {{ $kat }}
                    </a>
                @endforeach
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($kegiatan as $item)
                    <article class="card-kegiatan reveal">
                        ...
                        <div class="p-5">
                            <div class="text-slate-500 text-xs mb-2 flex items-center gap-1">
                            ...
                            <a href="{{ route('kegiatan.show', $item->slug) }}"
                               class="text-amber-500 text-sm font-semibold hover:text-amber-400 transition-colors inline-flex items-center gap-1">
                                Baca Selengkapnya
        ...
        @else
            <div class="text-center py-20">
                ...
                <p class="text-slate-400">Kegiatan akan segera ditambahkan. Pantau terus ya!</p>
            </div>
        @endif
    </div>
</section>
