...
<div class="page-header">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <p class="text-blue-400 mt-3 max-w-xl">Kenali para pengurus BEM Polbis yang berkomitmen melayani seluruh mahasiswa.</p>
    </div>
</div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @foreach($struktur as $divisi => $anggota)
                <div class="mb-16 reveal">
                    ...
                    <div class="flex items-center gap-4 mb-8">
                        <div class="h-px flex-1 bg-gradient-to-r from-lime-500/50 to-transparent"></div>
                        <div class="flex items-center gap-2 px-5 py-2 rounded-full border border-lime-500/30 bg-lime-500/10">
                            <span class="w-2 h-2 rounded-full bg-lime-500"></span>
                            <span class="text-lime-500 font-bold text-sm">{{ strtoupper($divisi) }}</span>
                        </div>
                        <div class="h-px flex-1 bg-gradient-to-l from-lime-500/50 to-transparent"></div>
                    </div>
                            ...
                            <div class="struktur-card" style="width: 170px; flex-shrink: 0;">
                                ...
                                @else
                                    <div class="w-20 h-20 rounded-full bg-gradient-to-br from-blue-600 to-blue-700 flex items-center justify-center mx-auto mb-3"
                                        ...
                                        <span class="text-lime-500 font-bold text-2xl">{{ strtoupper(substr($org->nama, 0, 1)) }}</span>
                                    </div>
                                @endif
                                <div class="text-white font-semibold text-sm leading-tight">{{ $org->nama }}</div>
                                @if($org->nim)
                                    <div class="text-blue-500 text-xs mt-0.5">{{ $org->nim }}</div>
                                @endif
                                <div class="mt-2 px-3 py-1 rounded-full text-xs font-semibold bg-lime-500/10 text-lime-400 border border-lime-500/20 inline-block">
                                    {{ $org->jabatan }}
                                </div>
                            </div>
                        @endforeach
        ...
        @else
            <div class="text-center py-20">
                ...
                <p class="text-blue-400">Data struktur organisasi akan segera ditambahkan.</p>
            </div>
        @endif
    </div>
</section>
