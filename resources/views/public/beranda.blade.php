        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center py-20">
            ...
            <div class="fade-in-up">
                ...
                <div class="grid grid-cols-3 gap-4 mt-12">
                    <div class="stat-card">
                        <div class="text-2xl font-black text-amber-500 font-display" style="font-family: 'Plus Jakarta Sans', sans-serif;">{{ \App\Models\Kegiatan::published()->count() }}+</div>
                        <div class="text-slate-400 text-xs mt-1">Kegiatan</div>
                    </div>
                    <div class="stat-card">
                        <div class="text-2xl font-black text-amber-500 font-display" style="font-family: 'Plus Jakarta Sans', sans-serif;">{{ \App\Models\Struktur::active()->count() }}+</div>
                        <div class="text-slate-400 text-xs mt-1">Anggota</div>
                    </div>
                    <div class="stat-card">
                        <div class="text-2xl font-black text-amber-500 font-display" style="font-family: 'Plus Jakarta Sans', sans-serif;">1</div>
                        <div class="text-slate-400 text-xs mt-1">Periode</div>
                    </div>
                </div>
            </div>
            ...
            <div class="hidden lg:flex justify-center items-center">
                <div class="relative">
                    <div class="w-80 h-80 rounded-full bg-gradient-to-br from-amber-500/20 to-indigo-500/20 blur-3xl absolute inset-0"></div>
                    <div class="relative card-glass p-10 rounded-3xl text-center">
                        ...
                        @endif
                        <div class="text-slate-300 font-semibold text-lg">{{ $profil['nama_bem'] ?? 'BEM Polbis' }}</div>
                        <div class="text-amber-500 font-semibold text-sm">{{ $profil['nama_kampus'] ?? 'Politeknik Bisnis Digital Indonesia' }}</div>
                        <div class="mt-6 flex justify-center gap-3">
                            @if(!empty($profil['instagram']))
                                <a href="{{ $profil['instagram'] }}" target="_blank"
                                   class="w-10 h-10 rounded-full bg-white/10 hover:bg-amber-500/20 flex items-center justify-center transition-all hover:scale-110">
                                    <svg class="w-5 h-5 text-amber-500" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>
                                </a>
                            @endif
                        </div>
                    </div>
...
<section class="py-20 bg-navy-900">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-12 reveal">
            ...
            <p class="text-slate-400 mt-3 max-w-xl mx-auto">Berbagai program dan kegiatan yang telah dan sedang dijalankan oleh BEM Polbis.</p>
        </div>
                ...
                @foreach($kegiatan_terbaru as $item)
                    <article class="card-kegiatan reveal">
                        ...
                        <div class="p-5">
                            ...
                            <p class="text-slate-400 text-sm leading-relaxed line-clamp-3 mb-4">{{ $item->deskripsi }}</p>
                            <a href="{{ route('kegiatan.show', $item->slug) }}" class="text-amber-500 text-sm font-semibold hover:text-amber-400 transition-colors inline-flex items-center gap-1">
                                Baca Selengkapnya
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                                </svg>
        ...
        @else
            <div class="text-center py-16">
                ...
                <p class="text-slate-400">Belum ada kegiatan yang dipublish.</p>
            </div>
        @endif
    </div>
</section>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div class="reveal">
                ...
                <h2 class="section-title mt-4">Visi & <span>Misi</span><br>BEM Polbis</h2>
                <div class="divider-gold"></div>
                <p class="text-slate-400 text-base leading-relaxed mt-4">
                    {{ $profil['visi'] ?? '' }}
                </p>
...
<section class="py-20 bg-navy-900">
    <div class="max-w-3xl mx-auto px-4 text-center reveal">
        <div class="card-glass p-12 rounded-3xl">
            <div class="text-5xl mb-4">💬</div>
            <h2 class="section-title mb-4">Ada Pertanyaan<br>atau <span>Aspirasi?</span></h2>
            <p class="text-slate-400 mb-8">Hubungi kami dan sampaikan aspirasi kamu. BEM Polbis siap mendengar dan merespons setiap masukan dari mahasiswa.</p>
